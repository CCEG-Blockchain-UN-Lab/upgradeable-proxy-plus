 ...
 * @notice The SafeProxied contract acts as the parent contract to Proxy and Upgradeable with and creates space for
 * state variables, functions and events that will be used in the upgradeable system.
 *
 * @dev Both the Proxy and Upgradeable need to hae the target and initialized state variables stored in the exact
 * same storage location, which is why they must both inherit from Proxied. Defining them in the separate contracts
 * does not work.
 *
 ...
 * function. Target Upgradeable contracts can only be initialized once.
 */
contract ProxiedPlus is Ownable{
    address public checkContract;
